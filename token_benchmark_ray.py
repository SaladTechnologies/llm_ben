...
def metrics_summary(
    ...
    ##########################################################################################
    # 4vCPU   12GB    3090
    # 0.016 + 0.012 + 0.25 = 0.278 dollar per hour - high
    # 0.016 + 0.012 + 0.10 = 0.128 dollar per hour - batch
    #price_3090_second_rate_b = (0.016 + 0.012 + 0.10) / 3600

    # 4vCPU   12GB    4090
    # 0.016 + 0.012 + 0.30 = 0.328 dollar per hour - high
    # 0.016 + 0.012 + 0.18 = 0.208 dollar per hour - batch
    price_4090_second_rate_b = (0.016 + 0.012 + 0.18) / 3600

    node_num = 5

    temp_in  = df_without_errored_req[common_metrics.NUM_INPUT_TOKENS].sum()
    temp_out = df_without_errored_req[common_metrics.NUM_OUTPUT_TOKENS].sum()
    
    ret["duration_s"] = end_time - start_time
    print(f"duration_s: {end_time - start_time}")

    temp_c = node_num * (end_time - start_time) * price_4090_second_rate_b
    ret["infra_cost_$"] = temp_c
    print(f"infra_cost_$: {temp_c}")
 
    ret["total_input_tokens"] = temp_in
    print(f"total_input_tokens: {temp_in}") 

    ret["total_output_tokens"] = temp_out
    print(f"total_output_tokens: {temp_out}") 

    temp_p = 1000 * ret["infra_cost_$"] / num_completed_requests
    ret["cost_usd_per_1k_requests"] = temp_p
    print(f"cost_usd_per_1k_requests: {temp_p}")

    temp_out_p = 1000000 * ret["infra_cost_$"] / temp_out
    ret["output_cost_usd_per_1m_tokens"] = temp_out_p
    print(f"output_cost_usd_per_1m_tokens: {temp_out_p}")
